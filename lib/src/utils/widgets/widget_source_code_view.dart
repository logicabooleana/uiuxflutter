...
class CodeFileWidget extends StatefulWidget {
  ...
  Future<Navigator?> navigation(BuildContext context){
    return Navigator.push(context,MaterialPageRoute(builder: (context) => CodeFileWidget(recipeName:recipeName,codeFilePath:codeFilePath) ));
  }
  // [ START - Vista ButtonIcon ]
  IconButton getIConButtonView({required BuildContext context,Color color = Colors.white}) {
    return  IconButton(
      icon: Theme.of(context).brightness == Brightness.dark?  Icon(Icons.code, size: 24.0,color: color): Icon(Icons.code,size: 24.0,color: color), 
      onPressed: (){ 
        Navigator.push(context,MaterialPageRoute(builder: (context) => CodeFileWidget(recipeName:recipeName,codeFilePath:codeFilePath)));
          ...
          if (snapshot.hasData) {
            return Scaffold(
              body: Padding(
                ...
                child: SourceCodeView(titleCode:widget.recipeName ,filePath:widget.codeFilePath), 
              ),
            );
          } else {
      ...
      FloatingActionButton(
        ...
        onPressed: () async {
          ...
          if (await Permission.storage.isGranted) {
            ...
            saveStorage(filename: "${widget.recipeName}.txt",content: sContentCode);
          }else{
